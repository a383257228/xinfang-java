		...
		this.autocomplete = function(param){
			...
			this.config = {
               ...
               //callback：选中行后按回车或单击时回调的函数
            	callback:null,
               //onMakeContAndShow:下拉菜单加载完成后的回调
            	onMakeContAndShow:null
			};
			...
			$this.keyup(function(event) {
				...
				if(!isFunctionalKey && (!ctrl || (ctrl && k == 67) || (ctrl && k == 88)) ){
					...
					if(data != null && $.isArray(data) ){
						...
						if(loadComboTimeout){
							clearTimeout(loadComboTimeout);
						}
						loadComboTimeout = setTimeout(function(){
							//ajax请求数据
			...
			function makeContAndShow(data_){
				...
				for(var i=0;i<data_.length;i++){
					cont += "<tr><td><div class='optionsTitle'>" + data_[i].title + "</div><div class='optionsCount'>"+data_[i].result.count+"件</div></td></tr>"
				}
				cont += "</tbody></table>";
				...
				var onMakeContAndShow_ = $this.data("config").onMakeContAndShow;
				onMakeContAndShow_(data_);
			}			
